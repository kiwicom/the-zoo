...
def test_download_repository__project_does_not_exist(repo_dir, repository, mocker):
    ...
    with pytest.raises(uut.RepositoryNotFoundError):
        uut.download_repository(repository, repo_dir)

    m_get_project.assert_called_once_with(repository.remote_id)


@pytest.mark.parametrize("repository__provider", [item.value for item in Provider])
def test_download_repository__missing_files(repo_dir, repository, mocker):
    scm_module = get_scm_module(repository.provider)
    m_get_project = mocker.patch.object(
...
@pytest.mark.parametrize("repository__provider", [item.value for item in Provider])
def test_download_repository(repo_dir, repo_archive, repository, mocker):
    scm_module = get_scm_module(repository.provider)
    m_get_project = mocker.patch.object(
        scm_module, "get_project", return_value=mocker.sentinel.project
    )
    ...
    repo_path = uut.download_repository(repository, repo_dir)
...
@pytest.mark.parametrize("repository__provider", [item.value for item in Provider])
def test_download_repository__custom_sha(repo_dir, repo_archive, repository, mocker):
    scm_module = get_scm_module(repository.provider)
    m_get_project = mocker.patch.object(
        scm_module, "get_project", return_value=mocker.sentinel.project
    )
    ...
    sha = "idspispopd"
    repo_path = uut.download_repository(repository, repo_dir, sha)
    assert {f.name for f in repo_path.iterdir()} == {
        "readme.md",
        "requirements.txt",
        "package.json",
